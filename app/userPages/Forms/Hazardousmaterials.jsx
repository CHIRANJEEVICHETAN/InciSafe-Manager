...
export default function EChemicalIncident() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        Alert.alert("Error", "Failed to fetch data from the server. In hazardousmaterials");
      }
    };
    fetchData();
  }, []);
...
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const incidentCategory = "Hazardous Material Handling Incident";
      ...
      await setDoc(doc(db, "ChemicalIncident", customDocId), violationData); // Use setDoc with custom ID
      ...
      if (notificationResponse.status === 200) {
        ...
        await setDoc(doc(db, "ChemicalIncident", customDocId), { notificationSent: true }, { merge: true });
      }
  ...
  const renderContent = () => (
    <View style={styles.contentContainer}>
      ...
      <Text style={styles.header}>Hazardous Material Handling Incident</Text>
      ...
      />
      <TextInput
        placeholder="Enter Chemical Name and Volume"
        value={chemicalName}
        onChangeText={text => setChemicalName(text)}
  ...
  const violationTypesData = [
    { key: 'spill', label: 'Chemical Spill' },
    { key: 'ppe', label: 'Inadequate PPE' },
    { key: 'storage', label: 'Unsafe Chemical Storage' },
  ];
