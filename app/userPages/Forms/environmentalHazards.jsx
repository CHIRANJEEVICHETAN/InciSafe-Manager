...
export default function EnvironmentalHazard() {
  ...
  const [violationTypes, setViolationTypes] = useState({
    air: autoFill ? violationType === 'air' : false,
    noise: false,
    water: false,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        Alert.alert("Error", "Failed to fetch data from the server. In environmentalHazards" + error.message);
      }
    };
    fetchData();
  }, []);
...
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const incidentCategory = "Environmental Hazard Reporting Form";
      ...
      if (notificationResponse.status === 200) {
        ...
        await setDoc(doc(db, "EnvironmentalHazard", customDocId), { notificationSent: true }, { merge: true });
      }
  ...
  const renderContent = () => (
    <View style={styles.contentContainer}>
      ...
      <Text style={styles.header}>Environmental Hazard Reporting Form</Text>
  ...
  const violationTypesData = [
    { key: 'air', label: 'Air Quality Violation' },
    { key: 'noise', label: 'Noise Hazard' },
    { key: 'water', label: 'Water/Soil Contamination' },
  ];
