...
export default function PolicyViolation() {
  ...
  const [violationTypes, setViolationTypes] = useState({
    unauthorized: autoFill ? violationType === 'unauthorized' : false,
    tagout: false,
    safety: false,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        Alert.alert("Error", "Failed to fetch data from the server. In policyViolations");
      }
    };
    fetchData();
  }, []);
...
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const incidentCategory = "Work Policy Violation Incident";
      ...
      await setDoc(doc(db, "PolicyViolation", customDocId), violationData); // Use setDoc with custom ID
      ...
      if (notificationResponse.status === 200) {
        ...
        await setDoc(doc(db, "PolicyViolation", customDocId), { notificationSent: true }, { merge: true });
      }
  ...
  const renderContent = () => (
    <View style={styles.contentContainer}>
      ...
      <Text style={styles.header}>Work Policy Violation Incident</Text>
  ...
  const violationTypesData = [
    { key: 'unauthorized', label: 'Unauthorized Personnel' },
    { key: 'tagout', label: 'Failure To Follow Tagout Procedure' },
    { key: 'safety', label: 'Violation Of Safety Procedures' },
  ];
